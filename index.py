...
class Coin(pygame.sprite.Sprite): #스프라이트를 제작하기 위해 선언
    def __init__(self,speed=5,sizex=-1,sizey=-1):# 스프라이트 초기화
        pygame.sprite.Sprite.__init__(self,self.containers)
        self.images,self.rect = load_sprite_sheet('coin.png',4,1,sizex,sizey,-1) # 이미지 정의
        self.rect.bottom = int(0.98*height)
        self.rect.left = width + self.rect.width
        self.image = self.images[random.randrange(0,3)]
        self.movement = [-1*speed,0]
    ...
    while not gameQuit:
        ...
        while not gameOver:
            ...
            for C in coins: # 닿았을때 어떻게 되는지
                C.movement[0] = -1*gamespeed
                if pygame.sprite.collide_mask(playerDino,C):
                    playerDino.isDead = True  # 죽지 않게 수정할 예정
                    if pygame.mixer.get_init() != None:
